...
import { Box, Search} from '@alifd/next';
...
export const BlockPane = (props: BlockPaneProps) => {
    ...
    const fetchBlocks = async (search:any) => {
    ...
    console.log('看看block数据', blocks);
    if (!blocks?.length || !blocks || blocks.length === 0) {
                ...
                blocks?.map((item:any,index:any) =>
                    <div key={index}>
                        ...
                        <Box direction='row' wrap>
                            {
                                item.list?.map((subItem:any,subIndex:any) => <BlockCard key={subIndex} id={subItem.id} title={subItem.title} screenshot={subItem.screenshot || DEFAULT_SCREENSHOT} blockData={subItem}/>)
                            }
                        </Box>
                    </div>
                )
